...
const AdminEditBookPage = () => {
    ...
    const fetchData = async () => {
        const book = (await axiosInstance.get(`/books/${id}`)).data.data
        setTitle(book.title)
        setAuthor(book.author)
        setPublisher(book.publisher)
        setDate(moment(book.publication_date).format("yyyy-MM-DD"))
        setCategory(book.categories.map((item) => ({ id: item._id, title: item.title })))
                        ...
                        <div className="flex flex-col lg:flex-row">
                            ...
                            <div className="lg:max-w-[50%] lg:ml-2">
                                ...
                                <ImageUpload onUploadEnd={() => setLoading(false)} onUploadStart={() => setLoading(true)}
                                    onUploadSuccess={(image) => { console.log(image); setNewCover(image) }} aspectClass='aspect-[5/5]' image={newCover ? `/uploads/tmp/${newCover}` : cover ? `/uploads/${cover}` : null} />
                            </div>
                        </div>
